...
const BGM_TRACKS = [
    { file: '79拍:分1.mp3', bpm: 80 },
    { file: '79拍:分2.mp3', bpm: 80 },
    { file: '90拍:分.mp3', bpm: 92 },
    { file: '110拍:分.mp3', bpm: 108 },
    { file: '103(1).mp3', bpm: 104 },
    { file: '79(1).mp3', bpm: 80 },
    { file: '79(2).mp3', bpm: 80 },
    { file: '79(3).mp3', bpm: 80 },
    { file: '79(4).mp3', bpm: 80 },
    { file: '82(1).mp3', bpm: 83 },
    { file: '84(1).mp3', bpm: 85 },
    { file: '89(1).mp3', bpm: 88 },
    { file: '90(1).mp3', bpm: 92 },
    { file: '94(1).mp3', bpm: 95 },
    { file: '94(2).mp3', bpm: 95 },
    { file: '100(1).mp3', bpm: 102 },
    { file: '110(1).mp3', bpm: 108 },
    { file: '111(1).mp3', bpm: 112 },
    { file: '128(1).mp3', bpm: 126 },
    { file: '132(1).mp3', bpm: 130 },
    { file: '132(2).mp3', bpm: 130 },
    { file: '134(1).mp3', bpm: 135 },
];
...
class Renderer {
    ...
    renderBackground(ctx, scrollX, w, h, images, beatPhase = 0) {
        ...
        if (groundSky) {
            ...
            if (tree) {
                // 木を大きく表示し、リズムに合わせて縦に揺れさせる（伸び縮み）
                const treeW = 220, treeH = treeW * (tree.height / tree.width);
                const spacing = 320;
                const toffset = -(scrollX * 0.5) % spacing;
                // 縦揺れ（伸び縮み）を強化：beatPulseを使って木の高さを変える
                const treeScale = 1 + beatPulse * 0.12;
                const scaledTreeH = treeH * treeScale;
                const scaledTreeW = treeW * (1 - beatPulse * 0.04); // 高さが伸びると幅は少し縮む
                for (let x = toffset - spacing; x < w + spacing; x += spacing) {
                    ctx.drawImage(tree, x, CONSTANTS.GROUND_Y - scaledTreeH, scaledTreeW, scaledTreeH);
                }
            }
            return;
        }
...
class GameController {
    ...
    // ホストの権威的な状態(敵・他プレイヤー・ノーツ)を自分の表示に反映する
    applyWorldState(data) {
        ...
        // ノーツ情報を同期（ホスト以外のノーツが流れてこない問題の修正）
        if (data.rhythmState) {
            const rs = data.rhythmState;
            this.rhythm.swordNotes = (rs.swordNotes || []).map(n => ({
                id: n.id, beat: n.beat, type: n.type, hit: n.hit, missed: n.missed,
                corrupted: n.corrupted, isGiant: n.isGiant, giantStage: n.giantStage,
            }));
            this.rhythm.abilityNotes = (rs.abilityNotes || []).map(n => ({
                id: n.id, beat: n.beat, type: n.type, hit: n.hit, missed: n.missed, corrupted: n.corrupted,
            }));
            this.rhythm.defendNotes = (rs.defendNotes || []).map(n => ({
                id: n.id, beat: n.beat, type: n.type, hit: n.hit, missed: n.missed, corrupted: n.corrupted,
            }));
            this.rhythm.combo = rs.combo || 0;
            this.rhythm.score = rs.score || 0;
            this.rhythm.swordBurstActive = rs.swordBurstActive || false;
            this.rhythm.swordBurstStartBeat = rs.swordBurstStartBeat || 0;
            this.rhythm.swordBurstLength = rs.swordBurstLength || 0;
            this.rhythm.abilityActive = rs.abilityActive || false;
            this.rhythm.abilityStartBeat = rs.abilityStartBeat || 0;
            this.rhythm.abilityLength = rs.abilityLength || 0;
        }
    }

    // ホストは権威的な世界の状態(敵・全プレイヤー・ノーツ)を全参加者へ配信し、
    ...
    syncNetworkState() {
        if (this.network.role === 'host') {
            ...
            // ノーツ情報を同期（ホスト以外のノーツが流れてこない問題の修正）
            const rhythmState = {
                swordNotes: this.rhythm.swordNotes.map(n => ({
                    id: n.id, beat: n.beat, type: n.type, hit: n.hit, missed: n.missed,
                    corrupted: n.corrupted, isGiant: n.isGiant, giantStage: n.giantStage,
                })),
                abilityNotes: this.rhythm.abilityNotes.map(n => ({
                    id: n.id, beat: n.beat, type: n.type, hit: n.missed, missed: n.missed, corrupted: n.corrupted,
                })),
                defendNotes: this.rhythm.defendNotes.map(n => ({
                    id: n.id, beat: n.beat, type: n.type, hit: n.hit, missed: n.missed, corrupted: n.corrupted,
                })),
                combo: this.rhythm.combo,
                score: this.rhythm.score,
                swordBurstActive: this.rhythm.swordBurstActive,
                swordBurstStartBeat: this.rhythm.swordBurstStartBeat,
                swordBurstLength: this.rhythm.swordBurstLength,
                abilityActive: this.rhythm.abilityActive,
                abilityStartBeat: this.rhythm.abilityStartBeat,
                abilityLength: this.rhythm.abilityLength,
            };
            
            const payload = {
                type: 'worldState', enemies, players, rhythmState,
